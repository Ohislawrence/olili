            ...
            <div class="p-6 space-y-6">
              ...
              <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                  ...
                  >
                    <div class="flex items-center">
                      ...
                      <div>
                        ...
                        <div class="text-sm text-gray-500">Send to users with a role</div>
                      </div>
                    </div>
                  </button>
                  ...
                  <button
                    type="button"
                    @click="form.type = 'segment'"
                    class="p-4 border rounded-lg text-left transition-colors duration-150"
                    :class="form.type === 'segment' ? 'border-blue-500 bg-blue-50' : 'border-gray-300 hover:border-gray-400'"
                  >
                    <div class="flex items-center">
                      <ChartBarIcon class="h-5 w-5 text-gray-400 mr-3" />
                      <div>
                        <div class="font-medium text-gray-900">User Segment</div>
                        <div class="text-sm text-gray-500">Send based on behavior</div>
                      </div>
                    </div>
                  </button>
                  ...
                  >
                    <div class="flex items-center">
                      ...
                      <div>
                        ...
                        <div class="text-sm text-gray-500">Select specific users</div>
                      </div>
                    </div>
                  </button>
                </div>
              ...
              <!-- Segment Selection -->
              <div v-if="form.type === 'segment'" class="space-y-4">
                <div>
                  <label for="segment_type" class="block text-sm font-medium text-gray-700 mb-2">
                    Select User Segment
                  </label>
                  <select
                    id="segment_type"
                    v-model="form.segment_type"
                    @change="getSegmentCount"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  >
                    <option value="">Select a segment</option>
                    <option value="no_course_enrollment">Never Enrolled in Any Course</option>
                    <option value="no_course_completion">Enrolled But Never Completed Any Course</option>
                    <option value="no_exam_attempt">Never Attempted Exam Prep</option>
                    <option value="incomplete_courses">Started But Not Completed Specific Course</option>
                    <option value="recent_inactive">Inactive Users (No Activity)</option>
                    <option value="high_achievers">High Achievers (Score > X%)</option>
                    <option value="struggling_students">Struggling Students (Score < X%)</option>
                  </select>
                </div>

                <!-- Segment-specific fields -->
                <div v-if="form.segment_type === 'incomplete_courses'">
                ...
                <div v-if="form.segment_type === 'no_exam_attempt'">
                  <label for="exam_prep_id" class="block text-sm font-medium text-gray-700 mb-2">
                    Select Exam Prep
                  </label>
                  <select
                    id="exam_prep_id"
                    v-model="form.exam_prep_id"
                    @change="getSegmentCount"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  >
                    <option value="">Select an exam prep</option>
                    <option v-for="exam in examPreps" :key="exam.id" :value="exam.id">
                      {{ exam.name }}
                    </option>
                  </select>
                </div>

                <div v-if="form.segment_type === 'recent_inactive'">
                  <label for="days_inactive" class="block text-sm font-medium text-gray-700 mb-2">
                    Inactive for (days)
                  </label>
                  <input
                    id="days_inactive"
                    v-model="form.days_inactive"
                    type="number"
                    min="1"
                    max="365"
                    @change="getSegmentCount"
                ...
                <div v-if="form.segment_type === 'high_achievers'">
                  <label for="min_score" class="block text-sm font-medium text-gray-700 mb-2">
                    Minimum Score (%)
                  </label>
                  <input
                    id="min_score"
                ...
                <div v-if="form.segment_type === 'struggling_students'">
                ...
                <div v-if="segmentCount !== null" class="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                  <p class="text-blue-800">
                    <span class="font-bold">{{ segmentCount }}</span> users match this segment
                  </p>
                </div>
              </div>
...
const getSegmentCount = async () => {
  if (!form.segment_type) {
    segmentCount.value = null
    return
  }

  try {
    const response = await fetch(route('admin.email.segment-count'), {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        exam_prep_id: form.exam_prep_id,
        days_inactive: form.days_inactive,
    ...
    console.error('Failed to get segment count:', error)
  }
}
...
// Watch for segment type changes to reset count
watch(() => form.segment_type, () => {
  segmentCount.value = null
})
</script>
